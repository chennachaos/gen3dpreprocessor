...
!-------------------------------------------------------------------------
 subroutine jasonMetis( numberOfNodes, nodeSideArrayRegister, connectivityArray, &
                        numberOfPartitions, numberOfCutEdges, &
                        parallelisationColorArray, numEdges )
! use,intrinsic :: ISO_C_BINDING
 IMPLICIT NONE
 integer :: numberOfNodes, numEdges
 integer,pointer :: nodeSideArrayRegister(:), connectivityArray(:)
 integer :: numberOfPartitions, numberOfCutEdges
 integer,pointer :: parallelisationColorArray(:)
 integer*8 :: numberOfNodes64, numberOfPartitions64, options64(40)
 integer*8 :: numberOfCutEdges64
 integer*8,pointer :: connectivityArray64(:), nodeSideArrayRegister64(:)
 integer*8,pointer :: parallelisationColorArray64(:), vertexWeights64(:)
 integer*8,pointer :: edgeWeights64(:)
 real*8 ,pointer :: partitionWeighting64(:)
 integer  allocateStatus,ub, ii
 integer*8 :: numConstraints64
 real*8 :: constraintImbalance64

 call METIS_SetDefaultOptions( options64 )
 options64(17) = 1
! options64(6) = 255

 numConstraints64 = 1
 numberOfNodes64 = numberOfNodes
 numberOfPartitions64 = numberOfPartitions
 numberOfCutEdges64 = numberOfCutEdges
 constraintImbalance64 = 1.001

 ub = UBOUND( connectivityArray, 1 )
 print *,'Allocating connectivityArray64',ub
 allocate(connectivityArray64( ub ),stat=allocateStatus)
 if (allocateStatus /= 0) STOP "ERROR: Not enough memory for connectivityArray64"
 connectivityArray64(1:ub) = connectivityArray(1:ub)
 print *,'Done'

 ub = UBOUND(nodeSideArrayRegister,1)
 print *,'Allocating nodeSideArrayRegister64', ub
 allocate(nodeSideArrayRegister64( ub ),stat=allocateStatus)
 if (allocateStatus /= 0) STOP "ERROR: Not enough memory for nodeSideArrayRegister64"
 nodeSideArrayRegister64(1:ub) = nodeSideArrayRegister(1:ub)
 print *,'Done'

 ub = UBOUND(parallelisationColorArray,1)
 print *,'Allocating parallelisationColorArray64', ub
 allocate(parallelisationColorArray64( ub ),stat=allocateStatus)
 if (allocateStatus /= 0) STOP "ERROR: Not enough memory for parallelisationColorArray64"
 parallelisationColorArray64(1:ub) = 0
 print *,'Done'

 print *,'Allocating vertexWeights64', numberOfNodes
 allocate( vertexWeights64( numberOfNodes ),stat=allocateStatus)
 if (allocateStatus /= 0) STOP "ERROR: Not enough memory for vertexWeights64"
 vertexWeights64(1:numberOfNodes) = 1
 print *,'Done'

 ub = UBOUND( connectivityArray64, 1 )
 print *,'Allocating edgeWeights64', ub
 allocate( edgeWeights64( ub ),stat=allocateStatus)
 if (allocateStatus /= 0) STOP "ERROR: Not enough memory for edgeWeights64"
 edgeWeights64(1:ub) = 1
 print *,'Done'

 print *,'Allocating partitionWeighting64', ub
 allocate( partitionWeighting64( numberOfPartitions ),stat=allocateStatus)
 if (allocateStatus /= 0) STOP "ERROR: Not enough memory for edgeWeights64"
 partitionWeighting64(1:numberOfPartitions) = 1.0 / numberOfPartitions
 print *,'Done'

 print *,'Calling Metis'
 call METIS_PartGraphKway( numberOfNodes64, numConstraints64, &
                           nodeSideArrayRegister64, connectivityArray64, &
                           vertexWeights64, vertexWeights64, &
                           edgeWeights64, &
                           numberOfPartitions64, partitionWeighting64, &
                           constraintImbalance64, options64, &
                           numberOfCutEdges64, parallelisationColorArray64 )
 print *,'Copying back parallelisationColorArray64'
 ub = UBOUND(parallelisationColorArray,1)
 parallelisationColorArray(1:ub) = parallelisationColorArray64(1:ub)

print *,'Deallocating connectivityArray64'
 deallocate( connectivityArray64, stat=allocateStatus )
 if (allocateStatus /= 0) STOP "ERROR: Couldn't deallocate connectivityArray64"

 print *,'Deallocating nodeSideArrayRegister64'
 deallocate( nodeSideArrayRegister64, stat=allocateStatus )
 if (allocateStatus /= 0) STOP "ERROR: Couldn't deallocate nodeSideArrayRegister64"

 print *,'Deallocating parallelisationColorArray64'
 deallocate( parallelisationColorArray64, stat=allocateStatus )
 if (allocateStatus /= 0) STOP "ERROR: Couldn't deallocate parallelisationColorArray64"

 print *,'Deallocating vertexWeights64'
 deallocate( vertexWeights64, stat=allocateStatus )
 if (allocateStatus /= 0) STOP "ERROR: Couldn't deallocate vertexWeights64"

 print *,'Deallocating edgeWeights64'
 deallocate( edgeWeights64, stat=allocateStatus )
 if (allocateStatus /= 0) STOP "ERROR: Couldn't deallocate edgeWeights64"

 print *,'Deallocating partitionWeighting64'
 deallocate( partitionWeighting64, stat=allocateStatus )
 if (allocateStatus /= 0) STOP "ERROR: Couldn't deallocate partitionWeighting64"

 end subroutine jasonMetis
!-------------------------------------------------------------------------
...
! colors nodes for parallelization using MeTiS
 ...
 type(SideRegisterData) :: srd
    ...
    if(numberOfPartitions>1) then
      ...
      call jasonMetis(srd%numberOfNodes,srd%nodeSideArrayRegister,connectivityArray,&
                   numberOfPartitions,numberOfCutEdges,&
                   srd%parallelizationColorArray, srd%numberOfSides)
